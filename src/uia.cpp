...
HWND uiaWnd = nullptr;
const char* WINDOW_CLASS_NAME = "REAPERCLARANotificationWND";

// Some UIA functions aren't available in earlier versions of Windows, so we
// must fetch those at runtime. Otherwise, ReaClassical Clara will fail to load. This class
...
HRESULT STDMETHODCALLTYPE UiaProvider::GetPropertyValue(PROPERTYID propertyId,
	...
	switch (propertyId) {
		...
		case UIA_ProviderDescriptionPropertyId:
			pRetVal->vt = VT_BSTR;
			pRetVal->bstrVal = SysAllocString(L"ReaClassical Clara");
			break;
		default:
			pRetVal->vt = VT_EMPTY;
	}
...
bool initializeUia() {
	...
	uiaWnd = CreateWindowEx(
		...
		WS_EX_TRANSPARENT,
		WINDOW_CLASS_NAME,
		"ReaClassical Clara Notifications",
...
bool shouldUseUiaNotifications() {
	static const bool cachedResult = []() -> bool {
		...
		const char setting = GetExtState("reaclassical_clara", "uiaNotificationEvents")[0];
		if (setting == '0') {
...
bool sendUiaNotification(const string& message, bool interrupt) {
	...
	return (uiaCore->RaiseNotificationEvent(
		...
		SysAllocString(L"REACLASSICAL_CLARA")
	) == S_OK);
}
